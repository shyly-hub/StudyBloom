...
export default function PostSessionLog({ navigation, route }) {
  ...
  const updateScore = sessCtx.updateScore || null;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // FIX 1: await addDoc before doing anything else — no data loss on fast navigation
      const ref = await addDoc(
        collection(db, 'users', user.uid, 'sessions'),
        sessionDoc
      );

      // FIX 2: pass serialisable date string, not serverTimestamp() — prevents
      // onSnapshot reconciliation crash that caused sessions to vanish on refresh
      if (addSession) addSession({ id: ref.id, ...sessionDoc, createdAt: new Date().toISOString() });
      ...
      // FIX 3: update sessionContext score immediately so Analytics/History
      // rings update without needing a refresh
      updateScore?.(newScore);

      // FIX 4: fire-and-forget so a network blip here doesn't fail the whole save
      ...
      }).catch(e => console.warn('[PostSessionLog] user doc update failed:', e));
